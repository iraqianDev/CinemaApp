<template>
  <div class="container mx-auto max-w-[95%]">
    <nav class="py-5 block text-center md:text-right md:flex  items-center   gap-2"
    >
      <router-link to="/">
        <h1 class="text-3xl font-bold text-red-600 flex-1 pb-2 mb-3 md:mb-0"
            @click="openSearch = false">Cinema</h1>
      </router-link>

      <div class="flex-1 relative ">
        <input type="text" class="w-full md:w-1/2  p-2 outline-none rounded relative" @keyup.enter="search"
               v-model="inputSearch"
               @input="debouncedSearch" @click="openSearch = true"
               placeholder="Search for movie!">
        <div v-if="inputSearch.length > 0 && openSearch"

             class="hideScroll md:w-1/2 absolute top-[110%] right-0 w-full bg-gray-800 rounded p-3 text-white overflow-y-scroll max-h-[80vh]">
          <div class="" v-if="movieSearch.length === 0">
            Loading...
          </div>
          <div class="rounded grid gap-2" v-else>
            ...
            <div
                class=" "
                v-for="movie in movieSearch" key="movie"
            >
              <router-link :to="{name: 'Movie Details', params: {id: movie.id}}" @click="openSearch = false"
                ...
                <div class="text-left w-full">
                  ...
                  <p class="w-[30ch] break-words truncate  text-xs">{{ movie.overview }}</p>
                </div>
              </router-link>
            </div>
          </div>
